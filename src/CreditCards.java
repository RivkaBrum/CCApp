...
public class CreditCards {
	...
	public Purchase getLargestPuchaseAllCards() {
		...
		Purchase currPurchaseWithAmnt;
		if (! allCards.isEmpty()) {
			for (int i=0; i< allCards.size(); i++)
			{
				if(allCards.get(i).hasPurchase()) {
					currPurchaseWithAmnt= allCards.get(i).getLargestPuchase().getAmount();
					if( currPurchaseWithAmnt> largest)
					{
						largest= currPurchaseWithAmnt;
